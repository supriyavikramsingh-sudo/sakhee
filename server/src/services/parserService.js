...
class ParserService {
  ...
  extractLabValues(text) {
    ...
    const patterns = {
      insulin_fasting: /(?:fasting\s+insulin|insulin\s+fasting)[\s:]+(\d+\.?\d*)\s*(miu\/l|μiu\/ml)?/i,
      glucose_fasting: /(?:fasting\s+glucose|glucose\s+fasting|fbs)[\s:]+(\d+\.?\d*)\s*(mg\/dl)?/i,
      testosterone_total: /(?:total\s+testosterone|testosterone\s+total)[\s:]+(\d+\.?\d*)\s*(ng\/dl)?/i,
    ...
    logger.info('Lab values extracted', { count: Object.keys(labValues).length })
    return labValues
  }
