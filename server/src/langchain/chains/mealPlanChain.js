...
class MealPlanChain {
  ...
  buildMealPlanPrompt(preferences, healthContext, enhancedContext) {
    ...
    // 0.5 KETO CRITICAL OVERRIDE (if applicable - HIGHEST PRIORITY FOR MACRO SAFETY)
    if (preferences.isKeto) {
      // Calculate macro targets dynamically
      const dailyMacros = calculateDailyMacros(targetCalories, true);
      const perMealMacros = calculatePerMealMacros(targetCalories, mealsCount, true);

      prompt += `0.5️⃣ 🔥 KETOGENIC DIET - CRITICAL OVERRIDE (ABSOLUTE PRIORITY):\n`;
      prompt += `   🚨 THIS IS A KETO PLAN - STANDARD PCOS RULES DO NOT APPLY\n`;
      prompt += `   🚨 YOU WILL SEE CONFLICTING MACRO RATIOS BELOW - IGNORE THEM!\n`;
      prompt += `   🚨 USE ONLY THESE DYNAMIC MACRO TARGETS CALCULATED FOR THIS USER:\n\n`;

      prompt += `   📊 CORRECT MACRO TARGETS (GENERATED BY MACRO CALCULATOR):\n`;
      prompt += `      • Daily Carbs: ${dailyMacros.macros.carbs.grams}g (${dailyMacros.macros.carbs.percentage}% of ${targetCalories} kcal)\n`;
      prompt += `      • Daily Protein: ${dailyMacros.macros.protein.grams}g (${dailyMacros.macros.protein.percentage}%)\n`;
      prompt += `      • Daily Fat: ${dailyMacros.macros.fat.grams}g (${dailyMacros.macros.fat.percentage}%)\n`;
      prompt += `      • Per-Meal Carbs: ${perMealMacros.macros.carbs.grams}g ±3% (${perMealMacros.macros.carbs.minGrams}g - ${perMealMacros.macros.carbs.maxGrams}g)\n`;
      prompt += `      • Per-Meal Protein: ${perMealMacros.macros.protein.grams}g ±3% (${perMealMacros.macros.protein.minGrams}g - ${perMealMacros.macros.protein.maxGrams}g)\n`;
      prompt += `      • Per-Meal Fat: ${perMealMacros.macros.fat.grams}g ±3% (${perMealMacros.macros.fat.minGrams}g - ${perMealMacros.macros.fat.maxGrams}g)\n\n`;

      prompt += `   🚫 ABSOLUTELY BANNED INGREDIENTS (ZERO TOLERANCE - CRITICAL FOR KETOSIS):\n`;
      prompt += `      ❌ ALL GRAINS: rice, poha (flattened rice), wheat, roti, chapati, paratha, bread, naan\n`;
      prompt += `      ❌ ALL MILLETS: ragi, bajra, jowar, foxtail millet (despite being healthy for PCOS, TOO HIGH CARB for keto)\n`;
      prompt += `      ❌ ALL LEGUMES: besan (chickpea flour), dal (any type), moong, toor, urad, masoor, chana, rajma\n`;
      prompt += `      ❌ ALL STARCHY FOODS: poha, upma (made from grain), sooji/suji (semolina), oats, quinoa\n`;
      prompt += `      ❌ STARCHY VEGETABLES: potato, sweet potato, yam, corn, peas, carrots\n`;
      prompt += `      ❌ HIGH-SUGAR ITEMS: sugar, jaggery, honey, dried fruits, high-sugar fruits (banana, mango, grapes)\n\n`;

      prompt += `   ✅ MANDATORY KETO SUBSTITUTIONS (USE THESE - NO EXCEPTIONS):\n`;
      prompt += `      • Rice/Poha → CAULIFLOWER RICE (pulse raw cauliflower in food processor)\n`;
      prompt += `      • Ragi/Bajra/Jowar → ALMOND FLOUR or COCONUT FLOUR (for rotis/dosas)\n`;
      prompt += `      • Besan → ALMOND FLOUR or EGG (for binding)\n`;
      prompt += `      • Dal/Lentils → PANEER, TOFU, CHICKEN, FISH, EGGS (keto proteins)\n`;
      prompt += `      • Potato → CAULIFLOWER, ZUCCHINI, TURNIP, RADISH\n`;
      prompt += `      • Sugar → STEVIA, ERYTHRITOL, MONK FRUIT\n\n`;

      prompt += `   📋 PRE-GENERATION KETO VALIDATION CHECKLIST (RUN BEFORE SELECTING ANY MEAL):\n`;
      prompt += `      For EVERY meal template you consider:\n`;
      prompt += `      1. ✓ Does it contain rice, poha, ragi, bajra, jowar, or any grain? → MUST SUBSTITUTE\n`;
      prompt += `      2. ✓ Does it contain besan, dal, lentils, or chickpeas? → MUST SUBSTITUTE\n`;
      prompt += `      3. ✓ Does it contain potato, corn, peas, or starchy vegetables? → MUST SUBSTITUTE\n`;
      prompt += `      4. ✓ After substitution, will carbs be < ${perMealMacros.macros.carbs.maxGrams}g per meal? → MUST VERIFY\n`;
      prompt += `      5. ✓ After substitution, will fat be > ${perMealMacros.macros.fat.minGrams}g per meal? → MUST VERIFY\n`;
      prompt += `      ❌ IF ANY CHECK FAILS → REJECT THIS MEAL, SELECT DIFFERENT TEMPLATE\n\n`;

      prompt += `   🔄 KETO MEAL ADAPTATION PROCESS (MANDATORY FOR NON-KETO TEMPLATES):\n`;
      prompt += `      STEP 1: Read meal template from RAG (e.g., "Ragi Dosa with Coconut Chutney")\n`;
      prompt += `      STEP 2: Identify non-keto ingredients (ragi = millet grain = HIGH CARB)\n`;
      prompt += `      STEP 3: Apply substitution (ragi → almond flour or coconut flour)\n`;
      prompt += `      STEP 4: Update meal name (→ "Almond Flour Dosa with Coconut Chutney")\n`;
      prompt += `      STEP 5: Calculate macros using RAG nutrition data\n`;
      prompt += `      STEP 6: Verify macros match targets (±3% tolerance)\n`;
      prompt += `      STEP 7: Add 2-3 tbsp ghee/coconut oil to reach fat target\n`;
      prompt += `      STEP 8: Final validation - ALL checks must pass!\n\n`;

      prompt += `   ⚠️ COMMON KETO MISTAKES TO AVOID:\n`;
      prompt += `      ❌ Using "Ragi Dosa" without substitution (ragi is a grain - BANNED)\n`;
      prompt += `      ❌ Using "Besan Chilla" without substitution (besan is chickpea flour - BANNED)\n`;
      prompt += `      ❌ Using "Poha Upma" without substitution (poha is flattened rice - BANNED)\n`;
      prompt += `      ❌ Using "Dal Tadka" without substitution (dal is lentils - BANNED)\n`;
      prompt += `      ❌ Using millets thinking they're keto-friendly (they're NOT - too high carb)\n`;
      prompt += `      ✅ CORRECT: "Cauliflower Rice Upma" (poha → cauliflower rice)\n`;
      prompt += `      ✅ CORRECT: "Almond Flour Dosa" (ragi → almond flour)\n`;
      prompt += `      ✅ CORRECT: "Paneer Tikka Masala" (dal → paneer)\n\n`;

      prompt += `   🎯 FINAL KETO RULE - READ THIS CAREFULLY:\n`;
      prompt += `      IF you see ANY grain, millet, dal, besan, poha, sooji, or starchy vegetable in a meal:\n`;
      prompt += `      → STOP and SUBSTITUTE using the rules above\n`;
      prompt += `      → DO NOT proceed with that meal until substitution is complete\n`;
      prompt += `      → UPDATE the meal name to reflect the substitution\n`;
      prompt += `      → VERIFY macros match the targets shown above\n`;
      prompt += `      This is NON-NEGOTIABLE for maintaining ketosis!\n\n`;
    }
